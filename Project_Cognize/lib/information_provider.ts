...
function getRelevantSystemFiles(componentPath: string, config: ProjectConfig): string[] {
...
function compressDeps(targetFile: string, isComponent: boolean, config: ProjectConfig): string {
    ...
    try {
        ...
        rawOutput.split('\n').forEach(line => {
            if (line.includes('使用しているモジュール')) currentSection = 'deps';
            else if (line.includes('使用しているファイル')) currentSection = 'used';
...
function truncateTscOutput(output: string, targetFile: string, maxErrors: number = 20): string {
...
function runTscCheck(targetFile, config) {
  return new Promise((resolve) => {
    ...
    exec('npx tsc --noEmit', { cwd: config.PROJECT_ROOT, timeout: config.TSC_TIMEOUT }, (error: Error | null, stdout: string, stderr: string) => {
      if (error) {
...
async function gatherAllInfo(targetFile: string, config: ProjectConfig): Promise<GatheredInfo> {
    if (!fs.existsSync(targetFile)) throw new Error(`対象ファイルが見つかりません: ${targetFile}`);

    const isComponent = isComponentFile(targetFile, config);
    if (isComponent) console.log(`✨ Componentファイルを検出 → 関連Systemを分析します\n`);
