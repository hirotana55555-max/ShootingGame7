...
exports.gatherAllInfo = gatherAllInfo;
...
function isComponentFile(filePath, config) {
    const normalizedTarget = path_1.default.relative(config.PROJECT_ROOT, filePath).replace(/\\/g, '/');
...
function getRelevantSystemFiles(componentPath, config) {
    ...
    const allSystemFiles = fs_1.default.readdirSync(config.SYSTEMS_DIR).filter(f => f.endsWith('.js') || f.endsWith('.ts'));
    ...
    const relevantSystems = allSystemFiles.filter(systemFile => {
        const systemPath = path_1.default.resolve(config.SYSTEMS_DIR, systemFile);
        try {
            const systemCode = fs_1.default.readFileSync(systemPath, 'utf8');
...
function compressDeps(targetFile, isComponent, config) {
    const queryScript = path_1.default.resolve(config.SCRIPT_DIR, 'query_index.js');
    if (!fs_1.default.existsSync(queryScript))
        return `DEPS_WARNING:file=${targetFile}|status=query_script_missing`;
    try {
        const rawOutput = (0, child_process_1.execSync)(`node "${queryScript}" deps "${targetFile}"`, { encoding: 'utf8', cwd: config.PROJECT_ROOT, timeout: config.DEPS_TIMEOUT });
        ...
        const compressed = `DEPS:target=${path_1.default.basename(targetFile)}|imports=${dependencies.join(',')}|used_by=${dependents.join(',')}`;
        console.log(`📦 依存関係圧縮: ${rawOutput.length}文字 → ${compressed.length}文字`);
        return compressed;
    }
    catch (e) {
        return `DEPS_ERROR:file=${path_1.default.basename(targetFile)}|error=${e.message.substring(0, 100)}`;
    }
}
...
function runTscCheck(targetFile, config) {
    return new Promise((resolve) => {
        ...
        (0, child_process_1.exec)('npx tsc --noEmit', { cwd: config.PROJECT_ROOT, timeout: config.TSC_TIMEOUT }, (error, stdout, stderr) => {
            if (error) {
...
function loadRelevantSystemSourceCodes(systemFiles, config) {
    ...
    return systemFiles.map(file => {
        const relativePath = path_1.default.relative(config.PROJECT_ROOT, path_1.default.resolve(config.SYSTEMS_DIR, file));
        try {
            const code = fs_1.default.readFileSync(path_1.default.resolve(config.SYSTEMS_DIR, file), 'utf8');
...
async function gatherAllInfo(targetFile, config) {
    ...
    const sourceCode = fs_1.default.readFileSync(targetFile, 'utf8');
    console.log(`📄 ソースコード読み込み完了: ${path_1.default.relative(config.PROJECT_ROOT, targetFile)}`);
    const compressedDeps = compressDeps(targetFile, isComponent, config);
    let tscResult = '[TSC CHECK RESULT: NOT_APPLICABLE]';
    if (/\.(ts|tsx)$/.test(targetFile) && fs_1.default.existsSync(path_1.default.join(config.PROJECT_ROOT, 'tsconfig.json'))) {
