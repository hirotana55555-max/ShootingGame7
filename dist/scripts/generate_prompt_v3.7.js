...
function compressDeps(targetFile, isComponent) {
    ...
    let dependents = [];
    try {
        const rawOutput = execSync(`node "${queryScript}" deps "${targetFile}"`, {
            encoding: 'utf8',
            cwd: PROJECT_ROOT,
        ...
        if (isComponent) {
            ...
            const systemContexts = relevantSystems.map(file => `game/systems/${file}[POTENTIAL_ECS_DEPENDENCY]`);
        ...
        if (e.killed)
            return `DEPS_ERROR:file=${targetFile}|error=timeout`;
        if (e.code === 'ENOENT')
            return `DEPS_ERROR:file=${targetFile}|error=not found`;
